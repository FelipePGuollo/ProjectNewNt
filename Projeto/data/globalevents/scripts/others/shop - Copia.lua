		...
		repeat
			...
			if player then
				...
				if offer_type == 'item' then
					local i, new_container = 0, player:getInbox():addItem(1990, 1, INDEX_WHEREEVER, FLAG_NOLIMIT)
					while i ~= 1 do
					new_container:addItem(item_id, item_count, INDEX_WHEREEVER, FLAG_NOLIMIT)
					i = i + 1
					player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'The item '.. item_name ..' is in your mailbox, check.')
