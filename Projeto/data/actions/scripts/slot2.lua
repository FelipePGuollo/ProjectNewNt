...
return k
  ...
  local l = choose('hp','mp','ml','melee','shield','dist', 'crit')
  local p = getper()
  ...
  elseif getSlotCount(nam) < conf.maxSlotCount then
  local l = choose('hp','ml','melee','shield','dist', 'crit')
  local p = getper()
  doSendMagicEffect(getCreaturePosition(cid), 30)
  nam = nam..' ['..l..'.+'..p..'%]'
  doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR,l..'.+'..p..'%')
